...
class HeatmapGenerator:
    def __init__(self, path_model, nn_class_count, trans_crop, device=None):
        ...
        model.head = nn.Linear(num_ftrs, nn_class_count)
        ...
        try:
            checkpoint = torch.load(
                path_model, map_location=self.device, weights_only=True
            )
            ...
            try:
                ...
                if scalar_obj is not None and hasattr(
                    torch.serialization, "safe_globals"
                ):
                    with torch.serialization.safe_globals([scalar_obj]):
                        checkpoint = torch.load(
                            path_model, map_location=self.device, weights_only=False
                        )
                elif scalar_obj is not None and hasattr(
                ...
                raise RuntimeError(
                    f"Failed to load checkpoint {path_model}: {e_full}\nOriginal error: {e_weights}"
                ) from e_full
        ...
        missing = [k for k in model_state if k not in ck_clean]
        unexpected = [
            k
            for k in checkpoint_state
            if (k.startswith("module.") and k[len("module.") :] not in model_state)
            or (not k.startswith("module.") and k not in model_state)
        ]

        print(f"[Checkpoint loader] path={path_model}")
        print(
            f"  Format: {'weights-only' if used_weights_only else 'full-checkpoint/fallback'}"
        )
        ...
        for ck_cleaned, orig_ck in ck_clean.items():
            if (
                ck_cleaned in model_state
                and isinstance(checkpoint_state[orig_ck], torch.Tensor)
                and model_state[ck_cleaned].shape == checkpoint_state[orig_ck].shape
            ):
        ...
        if len(compatible_state) > 0:
            ...
            print(
                f"[Checkpoint loader] Loaded {len(compatible_state)} tensors into model (partial/lenient)"
            )
        else:
            try:
                ...
                print(
                    f"[Checkpoint loader] Strict load failed: {e}. Using model default state (no tensors loaded)"
                )
        ...
        self.transform = transforms.Compose(
            [
                ...
                transforms.Resize((trans_crop, trans_crop)),
                transforms.ToTensor(),
    ...
    def generate(self, path):
        ...
        cam = self.cam(tensor)
        cam = cv2.resize(cam, (self.trans_crop, self.trans_crop))

        # original image
        img = cv2.imread(path)
        img = cv2.resize(img, (self.trans_crop, self.trans_crop))
...
if __name__ == "__main__":
    ...
    # 4. RUN TEST
    # =========================
    path_input_image = "/kaggle/input/datasets/khanfashee/nih-chest-x-ray-14-224x224-resized/images-224/images-224/00000001_001.png"
    path_model = "/kaggle/input/datasets/anhquocnguyen123/swin-tiny-only-weight/best_auc_weights_only.pth"
    ...
    h = HeatmapGenerator(path_model, 14, 224)

    probs = h.predict(path_input_image)
    ...
    heatmap_img, bbox_img, crop_img = h.generate(path_input_image)
