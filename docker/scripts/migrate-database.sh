...
#   - Install the new NFD stack (install.sh handles that automatically)
#
# Note:
...
cleanup() {
    ...
    if [[ $exit_code -ne 0 ]]; then
        printf "\n${RED}[NFD]${NC} Migration data extraction failed (exit code %s).\n" "${exit_code}" >&2
        printf "${RED}[NFD]${NC} Full log: %s\n" "${LOG_FILE}" >&2
        printf "${YELLOW}[NFD]${NC} Your original data in '${OLD_VOLUME}' is untouched.\n" >&2
    fi
}
trap cleanup EXIT
...
docker volume ls --format '{{.Name}}' < /dev/null | grep -q "^${OLD_VOLUME}$" \
    || error "Legacy volume '${OLD_VOLUME}' not found.\n       Are you sure you ran the old all-in-one NFD container?"
...
if docker run --rm -v "${OLD_VOLUME}:/data" alpine \
    ...
    if $AUTO_YES; then
        ...
        printf "${YELLOW}[NFD]${NC} Enter the SECRET_KEY from your old container's environment\n"
        printf "${YELLOW}[NFD]${NC} (press Enter to generate a new one — existing sessions will be invalidated): "
...
info "and start the full NFD stack automatically."
printf "\n"
